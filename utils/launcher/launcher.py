...
if isGameRunning():
  ...
  try:
    ...
    #subprocess.run(f'{gamePath}.exe')

    app = Application(backend='win32').start(f'{gamePath}.exe')
    ...
    log('ERROR: Call of Dragons.exe not found, double check the game path and the config file')
